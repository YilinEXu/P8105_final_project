...
gif2 <- animate(genderperc +geom_point(aes(group = seq_along(year_reported)), size = 2)+ transition_reveal(as.numeric(year_reported)) + enter_fade() + exit_shrink(), fps = 20)
...
gif3 <- animate(gender_num + geom_point(aes(group = seq_along(year_reported))) + transition_reveal(as.numeric(year_reported)))
...
ggplot(compare_obese, aes(x = grade_level, y = percentage, fill = weight_status)) +
  geom_bar(stat = "identity",position = "dodge") +
  labs(title = "Overweight percentage compare to obsesity percentage",
       x = "Grade level", 
...
county20 = rbind(top25, colMeans = colMeans(top25)) %>%
...
county_20 = inner_join(county20,counties) %>%
  select(-percentage) %>%
  group_by(year) %>%
  mutate(rank = rank(-percent_overweight_or_obese),
