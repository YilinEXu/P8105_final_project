...
The main data set for our project is a data set from The Student Weight Status Category Reporting System (SWSCR). SWSCR collects weight status category data (underweight, healthy weight, overweight or obese, based on BMI-for-age percentile) and includes separate estimates of the percent of students overweight, obese and overweight or obese for all reportable grades within the county and/or region and by grade groups (elementary and middle/high). It can be access at [here](https://health.data.ny.gov/Health/Student-Weight-Status-Category-Reporting-Results-B/es3k-2aus). This dataset has a range from 2010 to 2019. 
...
combine %>% 
  plot_ly(x = ~median_income, y = ~percent_overweight_or_obese, type = 'scatter', mode = 'markers', color = ~county, marker = list(size = ~percent_overweight_or_obese, opacity = 0.5))


combine %>% 
  plot_ly(x = ~county, y = ~median_income, name = 'income', type = 'scatter', mode = 'lines') %>% 
  add_trace(y = ~percent_overweight_or_obese, name = 'obese', type = 'scatter', mode = 'lines+marker')

combine %>% 
  plot_ly(x = ~county, y = ~percent_overweight_or_obese, name = 'percent obesity', type = 'scatter') %>% 
  add_trace(income, y = ~income, type = 'scatter', mode = 'lines')
...
```{r}
# Further original data cleaning for graph of poverty and obesity
obesity_geo_df = 
  weight_df %>% 
  #filter(sex %in% "ALL") %>% 
  filter(!county == "STATEWIDE (EXCLUDING NYC)") %>% 
...
income_geo_df = left_join(coordinates, income, by = "county")  # combine income data to the main 
```
...
obesity_geo_plot = 
  ...
  plot_ly(
    ...
    color = ~percent_overweight_or_obese, alpha = 0.5
  )
...
income_geo_plot = 
  ...
  plot_ly(
    x = ~latitude, y = ~ longitude, type = "scatter", mode = "markers",
    color = ~median_income, alpha = 0.5
  )


fig1 = subplot(obesity_geo_plot, income_geo_plot)
