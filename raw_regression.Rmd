...
sex_df %>% 
  ...
  layout(xaxis = list(title = "Year reported"),
...
combine2 %>% 
  mutate(text_label = str_c("Overweight or obese: (%) ", percent_overweight_or_obese, "\nFood accessbility: (%)", food_insecurity_p)) %>% 
  ...
  layout(
    ...
    yaxis = list(title = "Food accessibility (%)"),
    xaxis = list(title = "Overweight or obese percent (%)")
  )
...
ay = list(tickfont = list(color = "black"), overlaying = "y", side = "right", title = "Race%")
