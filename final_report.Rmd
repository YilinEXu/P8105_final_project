...
For different trend graphs, the data was changed depends on the variables we want to look at. For example, when we want to look at the overweight and obesity trend for students by gender, we will filter with `gender = female & male`. Most of the data cleaning process are similar, except for the last plot which shows the comparison overweight or obesity percentage through the top 20 counties.

During the process, we found out some counties have more than one areas in their records. Some of these areas are not recording in every two year periods. Hence, we first drop the records that are not recording in every two-year periods and then dropout the rows having missing values. After cleaning the data, we found out there are still more than fifty counties remained, so we find out the mean value of all counties overweight or obesity percentage. After that, we pick up the mean value that are in top 20 ranking and showed them in the plot.
```{r, eval=FALSE}
counties <- maindata %>%
  filter(county != "STATEWIDE (EXCLUDING NYC)",
