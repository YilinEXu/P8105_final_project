...
We combined three extra datasets to examine the factors of economic status, food access ability, and racial difference in weight status. The economic status dataset m 2016 can be accessed [here](https://webbi1.health.ny.gov/SASStoredProcess/guest?_program=/EBI/PHIG/apps/chir_dashboard/chir_dashboard&p=it&ind_id=Ng100).
Food access ability dataset from 2015 can be accessed [here](https://webbi1.health.ny.gov/SASStoredProcess/guest?_program=/EBI/PHIG/apps/chir_dashboard/chir_dashboard&p=it&ind_id=Ng123).
Race dataset from 2010 can be accessed [here](https://labor.ny.gov/stats/nys/statewide-population-data.shtm).
...
Next, we are exploring the relationship grade level and overweight or obese percentage over the two years. 
...
Then, we explore distribution of the two years and overweight and obese percentage.
...
We are examine the distribution of median income and overweight or obese percentage. 
...
We are examine the distribution of food insecurity and overweight or obese percentage. 
...
Before proceeding, it is important to assess crude correlation among relevant variables, in case issues of multicollinearity arise during model development. Because our data contains categorical variable, we decided to use `model.matrix` to plot out the correlation graph. 
...
Of relevance to our hypotheses, the following variables were highly correlated and thus may not be accurately interpreted in a model as independent predictors:
  ...
  * `white` and `black` (*r*  =  -0.97)
  * `grade_levelMIDDLE/HIGH` and `grade_levelELEMENTARY` (*r*  =  -1)
  * `median_income` and `food_insecurity_p` (*r*  =  -0.93)

If only focusing on the correlation values between each predictor and the outcome of interest, we might predict that `grade_levelMIDDLE/HIGH`, `food_insecurity_p`, `sex`, and `white` have positive correlations with `percent_overweight_or_obese`, while `grade_levelELEMENTARY`, `median_income`, and `black` have negative correlations with `percent_overweight_or_obese`.
