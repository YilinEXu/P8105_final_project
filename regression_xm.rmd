...
```{r echo=FALSE}
# Normality check
qqnorm(log(linear_df$percent_overweight_or_obese), col = c("deepskyblue"))

hist(log(linear_df$percent_overweight_or_obese), main = "Histogram for log(percent overweight ot obese)", xlab = "log(percent overweight ot obese)", col = c("deepskyblue3"))
```

Above are the Q-Q plot and histogram of overweight/obesity data after log transformation.  Q-Q plot is perfectly in a straight line, histogram is normally distributed with a little left skewness.  Overall, the normality assumption is satisfied.  
